...
function applyRegex() {
  currentWordList = [...COMPLETE_WORD_LIST.matchAll(currentRuleRegex)].map(word => word[0]);
  updateDisplayedWordList();
}

function clearGuesses() {
  currentWordList = COMPLETE_WORD_LIST.split(" ");
  currentRules = {
    includes: [],
    excludes: [],
  };
...
function updateDisplayedWordList() {
  ...
  for (const word of currentWordList) {
    wordListDiv.innerHTML += '<div class="word">' + word + '</div>'
  }
}
